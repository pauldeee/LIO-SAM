...
POINT_CLOUD_REGISTER_POINT_STRUCT(LivoxPointXYZRTLT,
                                  ...
                                  (float, reflectivity, reflectivity) (uint8_t, tag, tag)
...
public:
    ...
    bool cachePointCloud(const sensor_msgs::PointCloud2ConstPtr&laserCloudMsg) {
        ...
        if (sensor == SensorType::VELODYNE) {
            ...
            for (size_t i = 0; i < tmpLivoxCloudIn->size(); i++) {
                ...
                auto&dst = laserCloudIn->points[i];
                if (i == 0)
                    firstPointTime = src.timestamp;
                dst.x = src.x;
                dst.y = src.y;
                dst.z = src.z;
                dst.intensity = src.reflectivity;
                dst.ring = src.line;
                dst.time = (src.timestamp - firstPointTime) * 1e-9f; // livox stamps pc2 with epoch
            }
        }
        else {
        ...
        if (ringFlag == 0) {
            ...
            if (ringFlag == -1) {
                ROS_ERROR("Point cloud ring or line channel not available, please configure your point cloud data!");
                ros::shutdown();
            }
        }
    ...
    bool deskewInfo() {
        ...
        if (imuQueue.empty() || imuQueue.front().header.stamp.toSec() > timeScanCur || imuQueue.back().header.stamp.
            toSec() < timeScanEnd) {
    ...
    PointType deskewPoint(PointType* point, double relTime) {
        ...
        if (firstPointFlag == true) {
            transStartInverse = (pcl::getTransformation(posXCur, posYCur, posZCur, rotXCur, rotYCur, rotZCur)).
                    inverse();
